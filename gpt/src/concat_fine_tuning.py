...
class Encoder___():
    ...
    def encode(self,input,input_len,past=None):
        with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
            lm_output = model.model(hparams=self.hparam, X=input, past=past, reuse=tf.AUTO_REUSE)
            presents = lm_output['present']
            presents.set_shape(model.past_shape(hparams=self.hparam, batch_size=None))
            target_mask = tf.sequence_mask(input_len, maxlen=tf.shape(input)[1], dtype=tf.float32)
...
class Decoder___():
    ...
    def decode_one_step(self,hparams,tokens,past):
    ...
    def decode_all(self,hparams,tokens,past):
        with tf.variable_scope(self.scope):
            lm_output = model.model(hparams=hparams, X=tokens, past=past, reuse=tf.AUTO_REUSE)
            logits = lm_output['logits']
            presents = lm_output['present']
...
class NMT_GPT():
    ...
    def def_placeholder_and_components(self):
        self.encoder = Encoder___('encoder', self.hparams)
        self.decoder = Decoder___('decoder', self.hparams)
    ...
    def build_training_model(self):
        ...
        for i in range(0,self.input_num):
            presents=self.encoder.encode(self.inputs[i],self.input_lens[i],past_for_decoder)
            if past_for_decoder is None:
                past_for_decoder=presents
            else:
                past_for_decoder=tf.concat([past_for_decoder,presents],axis=-2)
        all_logits=self.decoder.decode_all(self.hparams,tokens=self.target_in,past=past_for_decoder)['logits']
...
def concat_finetuning(domain='fr',max_len_limit=220,only_test=False):
    methods = ['ori', 'rule']
    model_path='gpt/models_cat_'+domain+'/'+'_'.join(methods)
#    init_model_path = 'gpt/models/formality_infer'
    if not os.path.exists('gpt/models_cat_'+domain):
        os.mkdir('gpt/models_cat_'+domain)
    ...
    test(config_path=config_path,input_num=len(methods),
         ...
         output_path='../evaluate/gyafc_model_outputs/' + domain + '_out/formal.gpt.cat_no_share.'+'_'.join(methods))


def domain_combined(test_domain,only_test=False):
    methods = ['ori', 'rule']
    model_path = './models_domain_combined/' + '_'.join(methods)
    #init_model_path = './models/formality_infer'
    if not os.path.exists('./models_domain_combined'):
        os.mkdir('./models_domain_combined')
    if not os.path.exists(model_path):
        os.mkdir(model_path)
        os.mkdir(model_path + '/formality_train')
        os.mkdir(model_path + '/formality_infer')
    data_path = '../training_data/domain_combined/'
    if not only_test:
        train(config_path=config_path, input_num=len(methods), sep_flag='\t', ori_gpt_model=init_model_path,
              train_corpus=data_path + 'train.' + '_'.join(methods),
              dev_corpus=data_path + 'val.' + '_'.join(methods),
    ...
    test(config_path=config_path, input_num=len(methods),
         model_dir=model_path + '/formality_infer',
         input_path='../training_data/dif_models_'+test_domain+'/eval.' + '_'.join(methods),
         output_path='../evaluate/gyafc_model_outputs/' + test_domain + '_out/formal.gpt.cat.domain_cmb.' + '_'.join(methods))
